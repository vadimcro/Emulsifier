...
def process_engine(cache, img_array, film_profile, log_e_grid, curve_data, 
    ...
    if cache.get('light_hash') == light_hash:
        ...
        # OPTIMIZATION: Single pass clamp and pow
        ...
        if light_on:
            ...
            if flat_amount > 0:
                ...
                # OPTIMIZATION: In-place operation (overwrites linear)
            ...
            if cross_amount > 0:
                ...
                # OPTIMIZATION: In-place zero clamp
            ...
            if bloom > 0:
                ...
                # OPTIMIZATION: In-place max clamp
    ...
    if cache.get('film_hash') == film_hash:
        ...
        # OPTIMIZATION: Combined in-place clamps
    ...
    # =========================================================================
    # LIVE NODES: COMPUTED ON THE FLY (4-9) - NO CACHING
    # =========================================================================

    # OPTIMIZATION: Fast combined clamp generating the live array
    ...
    if print_on:
        ...
        if contrast > 0:
            ...
            # OPTIMIZATION: In-place application to current_film_out
    ...
    # OPTIMIZATION: Final clamp before blending
    ...
    if levels_on:
        ...
        if bp > 0.0 or wp < 1.0 or mid_val != 1.0:
            # OPTIMIZATION: Overwrite final_out directly
