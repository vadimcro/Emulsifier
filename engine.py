...
def _tone_zone_weights(y):
    black = 1.0 - _smoothstep(0.04, 0.16, y)
    shadow = _smoothstep(0.06, 0.18, y) * (1.0 - _smoothstep(0.30, 0.44, y))
    mid = _smoothstep(0.24, 0.40, y) * (1.0 - _smoothstep(0.60, 0.76, y))
    high = _smoothstep(0.54, 0.70, y) * (1.0 - _smoothstep(0.82, 0.94, y))
    white = _smoothstep(0.82, 0.96, y)

    weights = np.stack([black, shadow, mid, high, white], axis=-1).astype(np.float32)
    weights /= np.sum(weights, axis=-1, keepdims=True) + 1e-6
    return weights
...
def _apply_input_conditioning(img_array, exposure_pct, luma_pct, source_fade_pct, vibrance_pct, tint_pct):
    ...
    if luma_amt > 0.0:
        ...
        target_range = 0.62
        zone_w = _tone_zone_weights(y)
        black_w, shadow_w, mid_w, high_w, white_w = [zone_w[:, :, i] for i in range(5)]
        pivots = (
            black_w * p05 +
            shadow_w * ((p05 + p50) * 0.5) +
            mid_w * p50 +
            high_w * ((p50 + p95) * 0.5) +
            white_w * p95
        )
        pivots = (pivots * 0.35) + (p50 * 0.65)

        if scene_range > target_range:
            raw_pressure = min((scene_range - target_range) / target_range, 1.0)
            pressure = 0.34 + (raw_pressure * 0.66)
            zone_bias = (
                black_w * 1.10 +
                shadow_w * 0.95 +
                mid_w * 0.42 +
            ...
            contrast_factor = 1.0 - (0.45 * luma_amt * pressure * zone_bias)
        else:
            raw_pressure = min((target_range - scene_range) / target_range, 1.0)
            pressure = 0.34 + (raw_pressure * 0.66)
            zone_bias = (
                black_w * 0.22 +
                shadow_w * 0.58 +
            ...
            contrast_factor = 1.0 + (0.35 * luma_amt * pressure * zone_bias)

       
        y2 = np.clip((y - pivots) * contrast_factor + pivots, 0.0, 1.0)
...
def process_engine(cache, img_array, film_profile, log_e_grid, curve_data, 
    ...
    if cache.get('film_hash') == film_hash:
        ...
        density_denom = np.maximum(t_max_3d - t_min_3d, 1e-5)
        
        out_linear = ne.evaluate("(transmittance - t_min_3d) / density_denom")
    ...
    if levels_on:
        ...
        if bp > 0.0 or wp < 1.0 or mid_val != 1.0:
            ...
            if mid_val != 1.0:
                safe_mid = max(float(mid_val), 1e-3)
                inv_mid = 1.0 / safe_mid
