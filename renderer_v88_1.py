...
try:
    ...
    messagebox.showerror(
        ...
        "Emulsifier v1.88 requires 'numexpr' for CPU hardware acceleration.\n\n"
...
try:
    ...
    messagebox.showerror(
        ...
        "Emulsifier v1.88 requires 'tkinterdnd2' and 'customtkinter'.\n\n"
        "Please open your terminal and run:\npip install tkinterdnd2 customtkinter"
    )
    sys.exit(1)

# --- TAICHI GPU ACCELERATION (NEW) ---
try:
    import taichi as ti
...
def lensflare_hidden_sun(uv: tm.vec2, pos: tm.vec2, time: ti.f32) -> tm.vec3:
    ...
    # f0 (the sun) is completely removed here as requested.
...
def apply_gpu_lens_flare(
    ...
    # Pre-calculate constants outside the pixel loop for maximum performance
    inv_w = 1.0 / master_w
    inv_h = 1.0 / master_h
    ...
    for y, x in ti.ndrange(img.shape[0], img.shape[1]):
        ...
        flare_col = flare_col * (strength * 3.0) # Bypass `cc` and boost
...
class FilmRendererApp(TkinterDnD.Tk):
    def __init__(self):
        super().__init__()
        self.base_title = "Emulsifier Film Emulator - Rendering Engine v1.88"
        ...
        self.hovered_btn = None # Used for tracking bottom-left button hover states
        ...
        self.flare_u_var = ctk.DoubleVar(master=self, value=1.5) # Initialized off-screen
    ...
    def paste_image(self, event=None):
        try:
            ...
            if isinstance(img, Image.Image):
                ...
                # Handles edge case where user copies a file from OS Explorer instead of raw pixels
                file_path = img[0]
            ...
            # Dynamic App Title for Clipboard
            mode_str = "GPU" if cv2.ocl.useOpenCL() else "CPU"
            file_size_mb = (w * h * 3) / (1024 * 1024) # Approximate memory size
    ...
    def setup_ui(self):
        ...
        for canvas in (self.canvas_left, self.canvas_right):
            ...
            canvas.bind("<Leave>", self.on_canvas_leave) # QoL Hover reset fix
    ...
    def apply_state(self, state):
        ...
        self.flare_amt_var.set(state['flare_amt'])
        self.flare_u_var.set(state['flare_u'])
        self.flare_v_var.set(state['flare_v'])
        ...
        self.pane_flare.switch_var.set(state['flare_on'])
    ...
    def update_canvases(self):
        ...
        # QoL 3: Interactive Hover Button Colors
    ...
    def on_canvas_hover(self, event):
        # --- NEW: Prevent hover from overwriting the crosshair ---
        ...
        # QoL 3: Determine which button is currently hovered
    ...
    def on_canvas_leave(self, event):
        # Clears hover state when mouse fully leaves the canvas area
    ...
    def load_image_from_path(self, file_path):
        try:
            ...
            try:
                ...
                mode_str = "GPU" if cv2.ocl.useOpenCL() else "CPU"
                # QoL 1: Enhanced dynamic App title
