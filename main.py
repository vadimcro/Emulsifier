...
from PIL import Image, ImageTk, ImageGrab, ImageCms
import numpy as np
import cv2
import json
import csv
...
class FilmRendererApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        if getattr(sys, 'frozen', False):
            if platform.system() == "Darwin" and ".app" in sys.executable:
                # Mac .app bundles bury the executable in Contents/MacOS/
                # We go up 3 levels to get back to the folder containing the .app
                mac_executable_dir = os.path.dirname(sys.executable)
                self.base_dir = os.path.abspath(os.path.join(mac_executable_dir, "../../.."))
            else:
                # Standard Windows/Linux .exe behavior
        ...
        self.export_result_queue = queue.Queue()
        ...
        self.osd_timer = None
        self._profile_attention_after_id = None
        self._profile_attention_phase = 0

        self.panes = [] 

        self.str_var = ctk.DoubleVar(master=self, value=65)
        self.input_exp_var = ctk.DoubleVar(master=self, value=20)
        self.input_luma_var = ctk.DoubleVar(master=self, value=25)
        self.input_vib_var = ctk.DoubleVar(master=self, value=20)
        self.input_tint_var = ctk.DoubleVar(master=self, value=0) 
        self.soft_var = ctk.DoubleVar(master=self, value=10)
        self.ca_var = ctk.DoubleVar(master=self, value=1)
        self.input_fade_var = ctk.DoubleVar(master=self, value=0)
    ...
    def _poll_results(self):
        ...
        try:
            while not self.result_queue.empty():
                ...
                if res.get('_render_generation') != self._render_generation:
                    continue
                if 'error' in res:
        ...
        try:
            while not self.export_result_queue.empty():
                success, export_profile_name = self.export_result_queue.get_nowait()
                self._export_complete(success, export_profile_name)
        except queue.Empty:
            pass
        except Exception as e:
            print(f"Export completion UI error: {e}")
            self.is_exporting = False
    ...
    # --- ICC COLOR HANDLING ---
    def _get_srgb_icc_profile(self):
        if self._srgb_icc_profile is None:
            profile = ImageCms.createProfile("sRGB")
            self._srgb_icc_profile = ImageCms.ImageCmsProfile(profile).tobytes()
        return self._srgb_icc_profile
    ...
    def _convert_to_srgb(self, img):
        ...
        if icc_profile:
            try:
                ...
                converted.info["icc_profile"] = self._get_srgb_icc_profile()
                return converted
            except Exception as e:
                print(f"sRGB conversion warning: {e}")
    ...
    def paste_image(self, event=None):
        try:
            ...
            if isinstance(img, Image.Image):
                self.original_image = self._convert_to_srgb(img)
    ...
    def confirm_reset(self):
        ...
        if messagebox.askyesno("Reset Workspace", "Are you sure you want to reset all applied settings?"):
            ...
            self._start_profile_attention()
            
            # --- ARCH FIX: Updated to match exact init defaults ---
            self.str_var.set(65)
            self.input_exp_var.set(20)
            self.input_luma_var.set(25)
            self.input_vib_var.set(20)
            self.input_tint_var.set(0)
            self.input_fade_var.set(0)
            ...
            self.pane_input.switch_var.set(True)
    ...
    def show_osd_message(self, message, duration=1000):
        ...
        self.osd_timer = self.after(duration, self.hide_osd_message)
    ...
    def _profile_needs_attention(self):
        return self.original_image is not None and self.film_profile is None

    def _start_profile_attention(self):
        if self._profile_attention_after_id is not None or not hasattr(self, "profile_dropdown"):
            return
        self._animate_profile_attention()

    def _stop_profile_attention(self):
        if self._profile_attention_after_id is not None:
            try:
                self.after_cancel(self._profile_attention_after_id)
            except Exception:
                pass
            self._profile_attention_after_id = None
        self._profile_attention_phase = 0
        if hasattr(self, "profile_dropdown"):
            self.profile_dropdown.configure(
                fg_color="#333333",
                button_color="#444444",
                button_hover_color="#555555"
            )

    def _animate_profile_attention(self):
        if not self._profile_needs_attention() or not hasattr(self, "profile_dropdown"):
            self._stop_profile_attention()
            return

        def blend_hex(base_hex, glow_hex, amount):
            base = tuple(int(base_hex[i:i + 2], 16) for i in (1, 3, 5))
            glow = tuple(int(glow_hex[i:i + 2], 16) for i in (1, 3, 5))
            mixed = tuple(int(base[i] + (glow[i] - base[i]) * amount) for i in range(3))
            return f"#{mixed[0]:02X}{mixed[1]:02X}{mixed[2]:02X}"

        cycle_steps = 32
        phase = (self._profile_attention_phase % cycle_steps) / cycle_steps
        glow_amount = 0.5 - 0.5 * np.cos(phase * np.pi * 2.0)
        glow_amount *= 0.72

        fg_color = blend_hex("#333333", "#46525F", glow_amount)
        button_color = blend_hex("#444444", "#7890A5", glow_amount)
        hover_color = blend_hex("#555555", "#8FA7BB", glow_amount)
        self.profile_dropdown.configure(
            fg_color=fg_color,
            button_color=button_color,
            button_hover_color=hover_color
        )
        self._profile_attention_phase += 1
        self._profile_attention_after_id = self.after(85, self._animate_profile_attention)

    def show_no_profile_warning(self):
        if getattr(self, '_showing_warning', False): 
            return
        self._showing_warning = True
        self.lbl_profile.configure(text="SELECT FILM PROFILE", text_color="#FFD700")
        self.show_osd_message("Select a Film Emulation Profile first")
        self._showing_warning = False

    def set_active_pane(self, active_pane):
        for pane in self.panes:
            pane.current_bg = "#3A3A3A" if pane.expanded else "#2A2A2A"
            if not pane.switch_var.get():
                pane.current_bg = "#202020"
            pane.header_frame.configure(fg_color=pane.current_bg) 
            pane.bypass_switch.set_bg(pane.current_bg) 
                
            if not pane.switch_var.get():
                pane.header_btn.configure(text_color="#777777")
            elif pane == active_pane:
    ...
    def setup_ui(self):
        ...
        ToolTip(self.profile_dropdown, "Choose a film profile to start rendering.")
        ...
        self.pane_input = CollapsiblePane(self.scroll_frame, "Input Conditioning", app=self, expanded=True)
        self.pane_input.pack(fill=tk.X, pady=(0, 2))
        self.panes.append(self.pane_input)
        self.lbl_input_exp = create_slider_row(
            self.pane_input.content_frame,
            self.pane_input,
            "Exposure Anchor",
            "Balances source brightness toward a stable midtone before film mapping.",
            self.input_exp_var,
            0,
            100
        )
        self.lbl_input_luma = create_slider_row(
            self.pane_input.content_frame,
            self.pane_input,
            "Scene Contrast Normalize",
            "Softly reduces harsh contrast or adds mild punch to flat images before film mapping.",
            self.input_luma_var,
            0,
            100
        )
        self.lbl_input_fade = create_slider_row(
            self.pane_input.content_frame,
            self.pane_input,
            "Source Fade",
            "Adds lifted blacks, softer contrast, weaker color separation, and a subtle aged-film cast.",
            self.input_fade_var,
            0,
            100
        )
        self.lbl_input_vib = create_slider_row(
            self.pane_input.content_frame,
            self.pane_input,
            "Vibrance Guard",
            "Tames excessive saturation while protecting normal skin and natural color.",
            self.input_vib_var,
            0,
            100
        )
        self.lbl_input_tint = create_slider_row(
            self.pane_input.content_frame,
            self.pane_input,
            "Tint Guard",
            "Gently reduces unwanted color cast using neutral midtones. Keep low for intentional warm/cool images.",
            self.input_tint_var,
            0,
            100
        )
        ...
        self.lbl_flat = create_slider_row(self.pane_light.content_frame, self.pane_light, "Contrast Flattening", "Softens contrast before light scatter and print shaping.", self.flat_var, 0, 100)
        self.lbl_cross = create_slider_row(self.pane_light.content_frame, self.pane_light, "Emulsion Color Crosstalk", "Simulates chemical dye impurities and complex hue twists.", self.cross_var, 0, 100)
        ...
        self.lbl_cont = create_slider_row(self.pane_print.content_frame, self.pane_print, "Print Contrast", "S-Curve contrast.", self.cont_var, 0, 100)
        self.lbl_subsat = create_slider_row(self.pane_print.content_frame, self.pane_print, "Film Saturation Compression", "Organically desaturates extreme highlights and shadows.", self.subsat_var, 0, 100)
        ...
        def toggle_flare_placement():
            ...
            self.show_osd_message("Flare placement active\nClick the image to set light source", duration=2400)
        ...
        self.lbl_grain_amt = create_slider_row(self.pane_grain.content_frame, self.pane_grain, "Grain Amount", "Adds visible film grain/noise intensity.", self.grain_amt_var, 0, 100)
        self.lbl_grain_size = create_slider_row(self.pane_grain.content_frame, self.pane_grain, "Grain Size", "Controls grain coarseness. Higher values make larger, softer grain.", self.grain_size_var, 1.0, 5.0, is_float=True)
        self.lbl_grain_chroma = create_slider_row(self.pane_grain.content_frame, self.pane_grain, "Grain Luma / Chroma Variation", "Controls colored grain. Low values are more monochrome; high values add dye-cloud color noise.", self.grain_chroma_var, 0, 100)
        ...
        self.lbl_ff_amt = create_slider_row(self.pane_edge.content_frame, self.pane_edge, "Edge Softness (Field Flatness)", "Softens detail near image edges.", self.ff_amt_var, 0, 100)
        self.lbl_ff_fall = create_slider_row(self.pane_edge.content_frame, self.pane_edge, "Edge Softness Reach", "Controls how far edge softness reaches toward the center.", self.ff_fall_var, 0, 100)
        ...
        self.set_active_pane(self.pane_input)
    ...
    def get_current_state(self):
        return {
            ...
            'input_exp': self.input_exp_var.get(),
            'input_luma': self.input_luma_var.get(),
            'input_vib': self.input_vib_var.get(),
            'input_tint': self.input_tint_var.get(),
            'input_fade': self.input_fade_var.get(),
            ...
            'wp': self.wp_var.get(),
            'input_on': self.pane_input.switch_var.get(),
    ...
    def apply_state(self, state):
        ...
        self.input_exp_var.set(state.get('input_exp', self.input_exp_var.get()))
        self.input_luma_var.set(state.get('input_luma', self.input_luma_var.get()))
        self.input_vib_var.set(state.get('input_vib', self.input_vib_var.get()))
        self.input_tint_var.set(state.get('input_tint', self.input_tint_var.get()))
        self.input_fade_var.set(state.get('input_fade', self.input_fade_var.get()))
        ...
        self.pane_input.switch_var.set(state.get('input_on', True))
    ...
    def update_labels(self):
        self.lbl_input_exp.configure(text=f"Exposure Anchor: {int(self.input_exp_var.get())}%")
        self.lbl_input_luma.configure(text=f"Scene Contrast Normalize: {int(self.input_luma_var.get())}%")
        self.lbl_input_vib.configure(text=f"Vibrance Guard: {int(self.input_vib_var.get())}%")
        self.lbl_input_tint.configure(text=f"Tint Guard: {int(self.input_tint_var.get())}%")
        self.lbl_input_fade.configure(text=f"Source Fade: {int(self.input_fade_var.get())}%")
        ...
        self.lbl_flat.configure(text=f"Contrast Flattening: {int(self.flat_var.get())}%")
        self.lbl_cross.configure(text=f"Emulsion Color Crosstalk: {int(self.cross_var.get())}%")
        ...
        self.lbl_cont.configure(text=f"Print Contrast: {int(self.cont_var.get())}%")
        self.lbl_subsat.configure(text=f"Film Saturation Compression: {int(self.subsat_var.get())}%")
        self.lbl_split.configure(text=f"Warm/Cool Split Tone: {int(self.split_var.get())}%")
        self.lbl_grain_amt.configure(text=f"Grain Amount: {int(self.grain_amt_var.get())}%")
        self.lbl_grain_size.configure(text=f"Grain Size: {self.grain_size_var.get():.1f}")
        self.lbl_grain_chroma.configure(text=f"Grain Luma / Chroma Variation: {int(self.grain_chroma_var.get())}%")
        ...
        self.lbl_ff_amt.configure(text=f"Edge Softness (Field Flatness): {int(self.ff_amt_var.get())}%")
        self.lbl_ff_fall.configure(text=f"Edge Softness Reach: {int(self.ff_fall_var.get())}%")
    ...
    def update_canvases(self):
        ...
        if self.original_image is None:
            ...
            self.canvas_left.create_text(
                ...
                fill="#555555",
                font=("Arial", max(12, int(15 * sf)), "normal")
            )
            return
    ...
    def on_canvas_hover(self, event):
        ...
        if new_hovered_btn != getattr(self, 'hovered_btn', None):
            ...
            self.update_canvases()
            hint = {
                "zoom": "Click image to zoom. Mouse wheel adjusts zoom.",
                "single": "Single processed preview. Hold / to compare original.",
                "sbs": "Side-by-side original and processed preview.",
                "wipe": "Wipe compare. Drag the vertical divider."
            }.get(new_hovered_btn)
            if hint:
                self.show_osd_message(hint, duration=750)
    ...
    def on_canvas_press(self, event):
        ...
        if getattr(self, 'is_placing_flare', False):
            ...
            self.canvas_left.config(cursor=cur)
            self.canvas_right.config(cursor=cur)
            self.show_osd_message("Flare source placed")
            self.trigger_render()
            self.save_state()
            return
    ...
    def _prepare_render_params(self):
        ...
        return {
            ...
            'flare_v': self.flare_v_var.get(),
            'input_on': self.pane_input.switch_var.get(),
            'input_exposure_pct': self.input_exp_var.get(),
            'input_luma_pct': self.input_luma_var.get(),
            'input_vibrance_pct': self.input_vib_var.get(),
            'input_source_fade_pct': self.input_fade_var.get(),
            'input_tint_pct': self.input_tint_var.get(),
            'optics_on': self.pane_optics.switch_var.get(),
            'light_on': self.pane_light.switch_var.get(),
            'print_on': self.pane_print.switch_var.get(),
            'grain_on': self.pane_grain.switch_var.get(),
    ...
    def trigger_render(self, event=None):
        ...
        params['_render_generation'] = self._render_generation
        params['_render_cache'] = self._render_cache
    ...
    def _render_worker(self):
        while True:
            ...
            render_generation = params.pop('_render_generation', None)
            render_cache = params.pop('_render_cache', {})
 
            try:
                rendered_array, pre_levels_array, optimal_str, auto_bp, auto_wp = engine.process_engine(render_cache, **params)
                ...
                if render_cache.get('orig_hist_hash') != img_id:
                    render_cache['orig_hist'] = calc_hist(params['img_array'])
                    render_cache['orig_hist_hash'] = img_id

                    
                orig_hist = render_cache['orig_hist']
                ...
                self.result_queue.put({
                    '_render_generation': render_generation,
                    'pil': processed_pil,
                    'orig_hist': orig_hist,
                    'proc_hist': proc_hist,
                ...
                self.result_queue.put({'_render_generation': render_generation, 'error': str(e)})
    ...
    def export_image(self):
        if getattr(self, 'is_exporting', False):
            self.show_osd_message("Export Already Running")
            return
        ...
        export_profile_name = f"{self.film_profile['manufacturer']} {self.film_profile['film_name']}".upper()
        ...
        export_icc_profile = self._get_srgb_icc_profile()
        master_w, master_h = export_image.size
        ...
        threading.Thread(target=self._export_worker, args=(save_path, export_image, export_params, export_profile_name, export_icc_profile), daemon=True).start()

    def _export_worker(self, save_path, export_image, params, export_profile_name, export_icc_profile):

        try:
            params['img_array'] = np.array(export_image, dtype=np.float32) / 255.0
            ...
            save_kwargs = {"icc_profile": export_icc_profile} if export_icc_profile else {}

            ext = save_path.lower()
            if ext.endswith(".jpg") or ext.endswith(".jpeg"):
                rendered_img.save(save_path, quality=100, subsampling=0, **save_kwargs)

            elif ext.endswith(".png"):
                rendered_img.save(save_path, compress_level=1, **save_kwargs)

            elif ext.endswith(".tif") or ext.endswith(".tiff"):
                rendered_img.save(save_path, compression="tiff_deflate", **save_kwargs)

            else:
                rendered_img.save(save_path, **save_kwargs)
            ...
            self.export_result_queue.put((success, export_profile_name))

    def _export_complete(self, success, export_profile_name):
        self.is_exporting = False
        if hasattr(self, "btn_export"):
            self.btn_export.configure(
                state="normal",
                text="Export Complete" if success else "Export Failed"
            )
            def reset_export_button_text():
                if not getattr(self, "is_exporting", False) and hasattr(self, "btn_export"):
                    self.btn_export.configure(text="Export Full-Res Render")
            self.after(1800, reset_export_button_text)

        if success:
            self.lbl_profile.configure(text=f"SAVED: {export_profile_name}")
