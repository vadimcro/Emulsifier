...
class FilmRendererApp(TkinterDnD.Tk):
    def __init__(self):
        super().__init__()
        self.title("Emulsifier Film Emulator - Rendering Engine v1.77")
        ...
        self.bp_var = ctk.DoubleVar(value=0)
        self.mid_var = ctk.DoubleVar(value=1.0)
        self.wp_var = ctk.DoubleVar(value=255)
    ...
    def setup_ui(self):
        ...
        ctk.CTkButton(self.top_pinned, text="Load Digital / AI Photo", command=self.load_image_dialog, 
                      fg_color="#5C5C5C", hover_color="#8B7300", text_color="#FFFFFF", height=40,
                      font=ctk.CTkFont(family="Arial", size=14, weight="bold")).pack(fill=tk.X, pady=(0, 6))

        preset_names = self.refresh_and_load_presets()
        
        self.profile_var = ctk.StringVar(value="Select Film Emulation...")
        ...
        self.str_var = ctk.DoubleVar(value=65) 
        self.soft_var = ctk.DoubleVar(value=15)
        self.ca_var = ctk.DoubleVar(value=2)
        
        self.cross_var = ctk.DoubleVar(value=20) 
        self.flat_var = ctk.DoubleVar(value=25)
        self.hal_var = ctk.DoubleVar(value=20)
        self.bloom_var = ctk.DoubleVar(value=10)
        
        self.cont_var = ctk.DoubleVar(value=40)
        self.split_var = ctk.DoubleVar(value=15)
        self.subsat_var = ctk.DoubleVar(value=30) 
        
        self.grain_amt_var = ctk.DoubleVar(value=30)
        self.grain_size_var = ctk.DoubleVar(value=1.5)
        self.ff_amt_var = ctk.DoubleVar(value=0)
        self.ff_fall_var = ctk.DoubleVar(value=40)
        self.vig_amt_var = ctk.DoubleVar(value=0)
        self.vig_fall_var = ctk.DoubleVar(value=30)
    ...
    def update_labels(self):
        ...
        if "Calculating" not in self.lbl_str.cget("text"):
            current = self.lbl_str.cget("text")
            auto_tag = " (Auto)" if "(Auto)" in current else ""
            self.lbl_str.configure(text=f"Overall Physical Mix: {int(self.str_var.get())}%{auto_tag}")
    ...
    def _render_worker(self):
        ...
        while True:
            ...
            try:
                rendered_array, pre_levels_array, optimal_str = self.process_engine(self._render_cache, **params)
    ...
    def _generate_radial_mask(self, h, w, falloff_pct, master_w, master_h, offset_x, offset_y):
        y_coords = np.arange(offset_y, offset_y + h)
        x_coords = np.arange(offset_x, offset_x + w)
    ...
    def process_engine(self, cache, img_array, soft_amt, ca_amt, ff_amt, ff_fall, vig_amt, vig_fall, flatten_pct, 
                       cross_pct, hal_pct, bloom_pct, contrast_pct, subsat_pct, split_pct, grain_amt, grain_size, strength_pct, 
        ...
        if cache.get('light_hash') == light_hash:
            ...
            if light_on:
                ...
                if cross_amount > 0:
                    M = np.array([
                        [1.0 + cross_amount, -cross_amount, 0.0],
                        [0.0, 1.0 + cross_amount, -cross_amount],
                        [-cross_amount, 0.0, 1.0 + cross_amount]
                    ], dtype=np.float32)
                    linear = np.dot(linear, M.T)
                    linear = np.clip(linear, 0.0, None)

                bloom = bloom_pct / 100.0
                if bloom > 0:
                    hl_mask_wide = np.clip((linear - 0.5) / 0.5, 0, 1).astype(np.float32)
                    bloom_blur = cv2.GaussianBlur(hl_mask_wide, (0,0), virtual_width * 0.02)
        ...
        if cache.get('film_hash') == film_hash:
            ...
            out_r = np.interp(mapped_log_e[:, :, 0], self.log_e_grid, self.curve_data['r'])
            out_g = np.interp(mapped_log_e[:, :, 1], self.log_e_grid, self.curve_data['g'])
            out_b = np.interp(mapped_log_e[:, :, 2], self.log_e_grid, self.curve_data['b'])
        ...
        # --- AUTO-MIX: GLOBAL WASHOUT DETECTOR (v77 Fix) ---
        current_film_out = np.copy(film_out)
        current_film_out = np.clip(current_film_out, 0, 1)
        
        optimal_str = None
        if is_auto_mixing:
            # 1. Measure the global average luminance of the original digital image
            orig_luma = 0.299 * img_array[:,:,0] + 0.587 * img_array[:,:,1] + 0.114 * img_array[:,:,2]
            orig_global_avg = np.mean(orig_luma)
            
            optimal_str = 40.0 # Floor for the auto-mix
            
            # 2. Test descending mix strengths
            for test_str in [100.0, 90.0, 80.0, 70.0, 60.0, 50.0, 40.0]:
                test_opacity = (test_str / 100.0) * 0.50 # Applying the v72 internal 50% cap
                test_blended = (img_array * (1.0 - test_opacity)) + (current_film_out * test_opacity)
                
                # 3. Measure the new global average luminance
                test_luma = 0.299 * test_blended[:,:,0] + 0.587 * test_blended[:,:,1] + 0.114 * test_blended[:,:,2]
                test_global_avg = np.mean(test_luma)
                
                # 4. Check against the 6.0% tolerance ceiling
                if (test_global_avg - orig_global_avg) <= 0.06: 
                    optimal_str = test_str
                    break
            
            # Lock the strength for the rest of the render pipeline
            strength_pct = optimal_str

        # --- NODE 4: DARKROOM PRINT ---
        ...
        if cache.get('print_hash') == print_hash:
            ...
            if print_on:
                ...
                if contrast > 0:
                    k = 0.46 
                    n = 2.5  
                    x_n = np.power(np.clip(current_film_out, 1e-6, 1.0), n)
                    k_n = np.power(k, n)
                    nr_curve = x_n / (x_n + k_n)
                    max_val = 1.0 / (1.0 + k_n)
                    nr_curve = nr_curve / max_val
                    current_film_out = (current_film_out * (1.0 - contrast)) + (nr_curve * contrast)

                subsat_amount = (subsat_pct / 100.0) * 0.85 
                if subsat_amount > 0:
                    luma = 0.299 * current_film_out[:,:,0] + 0.587 * current_film_out[:,:,1] + 0.114 * current_film_out[:,:,2]
                    luma_3d = np.stack([luma]*3, axis=-1)
                    sat_mult = 1.0 - subsat_amount * np.power((2.0 * luma_3d - 1.0), 2.0)
                    sat_mult = np.clip(sat_mult, 0.0, 1.0)
                    current_film_out = luma_3d + (current_film_out - luma_3d) * sat_mult

                split = split_pct / 100.0
                if abs(split) > 0:
                    luma = cv2.cvtColor(current_film_out.astype(np.float32), cv2.COLOR_RGB2GRAY)
                    luma_3d = np.stack([luma]*3, axis=-1)
                    ...
                    if split > 0: 
                        ...
                        current_film_out = current_film_out * ((1.0 - split) + (color_map * split))
                    else: 
            ...
            blended = (img_array * (1.0 - opacity)) + (current_film_out * opacity)
            cache['print'] = blended
        ...
        if cache.get('levels_hash') == levels_hash:
            ...
            if levels_on:
                ...
                if bp > 0.0 or wp < 1.0 or mid_val != 1.0:
                    blended_levels = (blended_levels - bp) / (wp - bp + 1e-5)
                    blended_levels = np.clip(blended_levels, 0.0, 1.0)
                    if mid_val != 1.0:
                        blended_levels = np.power(blended_levels, 1.0 / mid_val)
        ...
        if grain_on:
            ...
            if grain > 0:
                ...
                luma_blend = cv2.cvtColor(final_out.astype(np.float32), cv2.COLOR_RGB2GRAY)
                ...
                final_out = final_out + (noise * grain_mask_3d * grain * 0.15)
        ...
        if edge_on:
            if ff_amt > 0:
                ...
                blurred_img = cv2.GaussianBlur(final_out, (0,0), max_sigma)
                final_out = (final_out * (1.0 - ff_mask)) + (blurred_img * ff_mask)

            if vig_amt > 0:
                vig_mask = self._generate_radial_mask(h, w, vig_fall, master_width, master_height, offset_x, offset_y)
                intensity = vig_amt / 100.0
                final_out = final_out * (1.0 - (vig_mask * intensity))

        return np.clip(final_out, 0, 1), np.clip(pre_levels_blended, 0, 1), optimal_str
