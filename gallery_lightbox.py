...
class GalleryLightbox(ctk.CTkToplevel):
    ...
    def _initial_load_worker(self):
        ...
        for idx, preset_name in enumerate(self.ordered_presets):
            ...
            pct = (idx + 1) / total
            self.after(0, lambda p=pct: self.progress_bar.set(p) if self.winfo_exists() else None)
            ...
            # Save the thumbnail. 'full_pil' is deliberately missing!
            self.previews[preset_name] = {'thumb_pil': thumb_pil}
            self.after(0, self._add_thumbnail_to_ui, preset_name, thumb_pil)
        ...
        if self._is_alive and self.ordered_presets:
            ...
            if first_full:
                self.previews[first_preset]['full_pil'] = first_full
        ...
        while self._is_alive:
            ...
            try:
                ...
                for preset in self.ordered_presets:
                    if preset in self.previews and 'full_pil' not in self.previews[preset]:
            ...
            if target_preset in self.previews and 'full_pil' not in self.previews[target_preset]:
                ...
                if full_pil:
                    self.previews[target_preset]['full_pil'] = full_pil
                    
                    # If this is the image the user is currently staring at (waiting for), update the UI instantly
                    if self.selected_preset == target_preset:
                        self.after(0, self.update_large_preview)
