...
try:
    ...
    ti.init(arch=ti.gpu) # Initializes the fastest available GPU backend
except ImportError:
    root = tk.Tk()
    root.withdraw()
    messagebox.showerror(
...
def lensflare_hidden_sun(uv: tm.vec2, pos: tm.vec2, time: ti.f32) -> tm.vec3:
    ...
    # f0 (the sun) is completely removed here
...
def apply_gpu_lens_flare(
    ...
    for y, x in ti.ndrange(img.shape[0], img.shape[1]):
        ...
        # 2. Additive Blending (Linear Dodge)
...
class FilmRendererApp(TkinterDnD.Tk):
    def __init__(self):
        super().__init__()
        self.base_title = "Emulsifier Film Emulator - Rendering Engine v1.89.1"
        ...
        self.grain_chroma_var = ctk.DoubleVar(master=self, value=0) 
    ...
    def setup_ui(self):
        ...
        self.btn_load_recipe.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))

        self.btn_save_recipe = ctk.CTkButton(self.recipe_frame, text="Save Recipe", command=self.save_recipe_dialog,
                                             fg_color="#2A2A2A", hover_color="#444444", text_color="#E0E0E0",
                                             font=ctk.CTkFont(family="Arial", size=12, weight="bold"))
        self.btn_save_recipe.pack(side=tk.RIGHT, fill=tk.X, expand=True, padx=(5, 0))
    ...
    def apply_state(self, state):
        ...
        # Safely fallback to current UI values if an old recipe is missing a key
    ...
    def on_canvas_hover(self, event):
        ...
        if event.widget == self.canvas_left:
            ...
            elif int(85*sf) <= event.x <= int(85*sf)+btn_w and y_off <= event.y <= y_off + btn_h:
                new_hovered_btn = "sbs"
            elif int(125*sf) <= event.x <= int(125*sf)+int(55*sf) and y_off <= event.y <= y_off + btn_h:
    ...
    def _prepare_render_params(self):
        ...
        return {
            ...
            'full_proxy_array': self.preview_array, 
    ...
    def process_engine(self, cache, img_array, soft_amt, ca_amt, ff_amt, ff_fall, vig_amt, vig_fall, flatten_pct, 
                       ...
                       flare_amt, flare_u, flare_v, flare_on,
        ...
        if cache.get('light_hash') == light_hash:
            ...
            if light_on:
                ...
                if cross_amount > 0:
                    ...
                    linear = cv2.merge([new_r, new_g, new_b])
        ...
        if cache.get('film_hash') == film_hash:
            ...
            density = cv2.merge([out_r, out_g, out_b])
        ...
        # --- AUTO-MIX: GLOBAL WASHOUT DETECTOR ---
        ...
        if cache.get('print_hash') == print_hash:
            ...
            if print_on:
                ...
                if subsat_amount > 0:
                    ...
                    current_film_out = cv2.merge([r, g, b])
        ...
        # --- NODE 5: LENS FLARE (GPU) ---
        flare_hash = hash((print_hash, flare_amt, flare_u, flare_v, flare_on, virtual_width, offset_x, offset_y, img_id))
        if cache.get('flare_hash') == flare_hash:
            final_out = cache['flare']
        else:
            final_out = np.copy(blended)
            ...
            cache['flare'] = final_out
            cache['flare_hash'] = flare_hash

        # --- NODE 6: PHYSICAL GRAIN (SPARSED STRUCTURAL) ---
        grain_hash = hash((flare_hash, grain_amt, grain_size, grain_chroma, grain_on, virtual_width))
        if cache.get('grain_hash') == grain_hash:
            final_out = cache['grain']
        else:
            if grain_on:
                ...
                if grain > 0:
                    ...
                    final_out = cv2.merge([r, g, b])
            
            cache['grain'] = final_out
            cache['grain_hash'] = grain_hash

        # --- NODE 7: EDGE IMPERFECTIONS ---
        edge_hash = hash((grain_hash, ff_amt, ff_fall, vig_amt, vig_fall, edge_on, virtual_width, offset_x, offset_y))
        if cache.get('edge_hash') == edge_hash:
            final_out = cache['edge']
        else:
            if edge_on and (ff_amt > 0 or vig_amt > 0):
                # Calculate Base Radial Distance Map ONCE per viewport state
                ...
                if ff_amt > 0:
                    ...
                    ff_mask = ne.evaluate("where(ff_mask > 0.0, ff_mask, 0.0)")
                    ff_mask = ne.evaluate("where(ff_mask < 1.0, ff_mask, 1.0)")
                    ff_mask = ne.evaluate("ff_mask * ff_mask * (3.0 - 2.0 * ff_mask)")
                ...
                if vig_amt > 0:
                    ...
                    vig_mask = ne.evaluate("where(vig_mask > 0.0, vig_mask, 0.0)")
                    vig_mask = ne.evaluate("where(vig_mask < 1.0, vig_mask, 1.0)")
                    vig_mask = ne.evaluate("vig_mask * vig_mask * (3.0 - 2.0 * vig_mask)")
            ...
            cache['edge'] = final_out
            cache['edge_hash'] = edge_hash

        # Capture pre-levels state for histogram
