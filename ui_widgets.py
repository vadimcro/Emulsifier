...
class BypassSwitch(tk.Canvas):
    ...
    def draw(self, *args):
        ...
        track_color = "#6F7F8F" if state else "#151515"
        knob_color = "#E6E6E6" if state else "#444444"
...
class CollapsiblePane(ctk.CTkFrame):
    def __init__(self, parent, title, app, expanded=False):
        ...
        self.switch_var.trace_add("write", self.update_state_visual)
    ...
    def update_state_visual(self, *args):
        enabled = self.switch_var.get()
        self.current_bg = "#3A3A3A" if self.expanded else "#2A2A2A"
        if not enabled:
            self.current_bg = "#202020"

        self.header_frame.configure(fg_color=self.current_bg)
        self.bypass_switch.set_bg(self.current_bg)
        self.header_btn.configure(text_color="#E0E0E0" if enabled else "#777777")
        self.content_frame.configure(fg_color="#222222" if enabled else "#1A1A1A")
