...
* **Histogram Grain Exclusion:** The secondary Stats Pass forces the `grain_on` parameter to `False` in the background. The Global Histogram now measures pure tonal distribution, preventing artificial high/low peaks generated by the grain matrix from muddying the scope.
* **Threading Race Condition Fix:** Corrected an asynchronous execution trap where the Auto-Mix algorithm updated the slider *after* the viewport had already rendered. The background thread now halts the viewport render, calculates the optimal threshold, updates the UI, and feeds the new value into the viewport render perfectly in sync.

**Major Physics Refactor:**
* **Structural Sparsity Curve (Grain):** Completely overhauled the grain math. Replaced a flattening exponent (`0.8`) with a high-contrast exponent (`1.5`). The engine now violently crushes low-level digital noise "fog" into absolute zero, forcing the grain to visually clump into distinct, punchy, sparse specs rather than a uniform sheet of synthetic TV static.
* **Linear Interpolation Reversion:** Switched the upscale interpolation logic inside the grain node to `cv2.INTER_LINEAR`. By avoiding smoothing convolutions, scaled-up grain arrays now maintain mathematically harsh, microscopic digital edges to simulate jagged silver-halide crystals.
* **Luma/Chroma Decoupling (The Confetti Killer):** Eliminated the "digital sensor noise" artifacting. The core noise matrix is now heavily desaturated via the new `Color Variation` slider (default 15%). The engine isolates the pure monochrome structure (silver halide density) and smoothly layers back independent RGB chrominance (chemical dye clouds), yielding authentic, physical film grain.
